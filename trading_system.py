...
class TradingSystem:
    ...
    async def execute_intelligent_trade(self) -> Optional[str]:
        """Execute trade based on best prediction with profit-optimized pipeline"""
        ...
        # Check zero-loss risk manager
        market = self.market.get_current_market()
        prediction_dict = {
            "direction": self.best_prediction.direction,
            "confidence": self.best_prediction.confidence,
            "type": self.best_prediction.type,
            "volatility": self.market.get_volatility() if hasattr(self.market, "get_volatility") else 0,
            "trend_strength": self.market.get_trend_strength() if hasattr(self.market, "get_trend_strength") else 0,
            "signal_agreement": self.analysis.get_signal_agreement() if hasattr(self.analysis, "get_signal_agreement") else 0
        }
        
        zl_can_trade, zl_reason = self.zero_loss_risk_manager.should_trade(prediction_dict, market)
        if not zl_can_trade:
            logger.warning(f"Trade blocked by zero-loss risk manager: {zl_reason}")
            return None
        
        # Calculate optimal position size using position sizer
        market_conditions = {
            "volatility": prediction_dict.get("volatility", 0.01),
            "trend_strength": prediction_dict.get("trend_strength", 0)
        }
        optimal_amount = self.position_sizer.get_optimal_size(
            self.best_prediction.confidence,
            market_conditions,
            use_kelly=True
        )
        
        # Apply zero-loss position sizing
        safe_amount = self.zero_loss_risk_manager.calculate_position_size(
            optimal_amount,
            self.best_prediction.confidence,
            market
        )
        
        trade_amount = max(safe_amount, self.settings.base_amount * 0.5)
        
        # Use execution optimizer for optimal timing
        async def do_execute(prediction):
        ...
        optimized_result = await self.execution_optimizer.execute_optimized(
            do_execute,
            self.best_prediction
        )
        
        contract_id = optimized_result if isinstance(optimized_result, str) else None
    ...
    async def _on_trade_complete(self, contract_id: str, profit: float):
        ...
        # Update position sizer for next trade
        self.position_sizer.update_trade_result(profit)
        
        # Update zero-loss risk manager
        trade_data = self.executor.get_trade(contract_id) or {}
        self.zero_loss_risk_manager.record_trade_result(
            profit,
            self.market.get_current_market(),
            trade_data.get("type", "MANUAL")
        )
        ...
        # Check zero-loss kill switch
        zl_metrics = self.zero_loss_risk_manager.get_risk_metrics()
        if zl_metrics.get("kill_switch"):
            self.bot_status = "STOPPED"
            system_logger.warning("Zero-loss kill switch triggered", daily_pnl=zl_metrics.get("daily_pnl"))
