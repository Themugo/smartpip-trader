...
def setup_routes(app: FastAPI, trading_system):
    ...
    async def update_settings(request: Request):
        ...
        trading_system.settings.update(data)
        return JSONResponse({"success": True})
    ...
    async def switch_market(request: Request):
        ...
        if not rate_limiter.is_allowed(client_id):
            raise HTTPException(status_code=429, detail="Rate limit exceeded")
        data = await request.json()
        trading_system.switch_market(data.get("market"))
        return JSONResponse({"success": True})
    ...
    async def manual_trade(request: Request):
        ...
        if not rate_limiter.is_allowed(client_id):
            raise HTTPException(status_code=429, detail="Rate limit exceeded")
        data = await request.json()
        trading_system.settings.base_amount = data.get("amount", 1)
    ...
    async def force_switch_market(request: Request):
        ...
        if market:
            trading_system.market_selector.force_switch(market)
            trading_system.switch_market(market)
            return JSONResponse({"success": True, "market": market})
        
        return JSONResponse({"success": False, "error": "Invalid market"})
